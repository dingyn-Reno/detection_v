...
dataloader.train = [
    ...
    for dataloader_id, use_rfs, use_cp, use_filter, dataset_name in [
        [0, True, False, True, "objects365_train"], # chengguan_full_shot_train # objects365_train_fixname
    ]
]
...
dataloader.tests = [
    L(build_detection_test_loader)(
        dataset=L(get_detection_dataset_dicts)(names="objects365_val", filter_empty=False),
...
dataloader.evaluators = [
    L(COCOEvaluator)(
        dataset_name="objects365_val", # chengguan_test # objects365_val_fixname
        tasks=("bbox",),
    ),
]
...
# )
train.init_checkpoint = (
    "output/APE/configs/city/city_13/city_58_debug_v/best_model.pth"
)
...
model.model_vision.num_classes = 2
...
for criterion, num_classes in zip(
    model.model_vision.criterion, [2] # [365]
):
...
train.eval_period = 200 # full=500,few=300
...
model.model_vision.prompts_path='/home/vis/dingyuning03/baidu/personal-code/dingyuning_APE/APE/obj_imgs'
# model.model_vision.mode='infer'
model.model_vision.cls_nums=2