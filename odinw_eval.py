...
def main(args):
    ...
    cls_nums = [5, 5, 26, 7, 3, 36, 6, 13, 1, 6, 8, 1, 4, 3, 2, 1, 2, 43,37, 2, 1,20, 1, 2, 30,1,1, 11,3, 2, 2, 15, 5,8, 1]
    assert len(odinw_dataset_metas) == len(odinw_test_dataset_names) == len(cls_nums)
    train_iters = [100, 400, 1600, 500, 300, 400,12000,250, 2000, 800, 33000, 4000, 4000, 5500, 120, 40, 60, 3000, 10, 10, 40, 14000, 3100, 8800, 2500, 500, 200, 31000, 430, 100, 150, 6400, 900, 1700, 550]
    eval_iters = [100, 400, 1600, 500, 300, 400,3000,250, 2000, 800, 10000, 4000, 4000, 3000, 120, 40, 60, 3000, 10, 10,40, 14000, 3100, 5000, 2500, 500, 200, 10000, 430, 100, 150, 3200, 900, 1700, 550]
    for train_name, test_name, num, train_iter, eval_iter in zip(odinw_dataset_metas, odinw_test_dataset_names,cls_nums, train_iters, eval_iters):
        
        cfg = LazyConfig.load(args.config_file)
        cfg = LazyConfig.apply_overrides(cfg, args.opts)
        # 数据集的修改
        cfg.dataloader.train[0].dataset.names = [train_name]
        cfg.dataloader.train[0].mapper.dataset_names = [train_name]
        cfg.dataloader.tests[0].dataset.names = test_name
        cfg.dataloader.evaluators[0].dataset_name = test_name
        cfg.model.model_vision.dataset_names = train_name
        cfg.model.model_vision.dataset_metas = train_name

        # 种类的修改
        cfg.criterion.num_classes = num
        cfg.model.model_vision.cls_nums = num

        # 训练轮数的修改
        cfg.train.max_iter = train_iter
        cfg.train.eval_period = eval_iter
